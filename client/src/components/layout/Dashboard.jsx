...
const Dashboard = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [userResponse, qrCodesResponse] = await Promise.all([
          axios.get("https://qr-code-generator-api-beta.vercel.app/api/users/me", {
            headers: { "x-auth-token": token },
          }),
          axios.get("https://qr-code-generator-api-beta.vercel.app/api/qrCodes", {
            headers: { "x-auth-token": token },
          }),
        ]);
  ...
  const handleDelete = async (id) => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`https://qr-code-generator-api-beta.vercel.app/api/qrCodes/${id}`, {
        headers: { "x-auth-token": token },
      });
